...
def visualize_atlas(dataRoot):
    ...
    global df, not_fin_points_prev, fin_points_prev, class_predictions_curr, model, model_data
    df = df_dict["df"]
    ...
    def select_point(clickData, n_clicks, class_val, fileName, not_fin_points, fin_points):
    ...
    def chart_3d(class_predictions_in, not_fin_points, fin_points, n_clicks, fileName):
        ...
        if 'calc-button' in changed_id:
            if not_fin_points and fin_points:
                ...
                not_fin_indices = [n["pointNumber"] for n in not_fin_points]
                lb_indices = np.concatenate((not_fin_indices, fin_indices), axis=0)
